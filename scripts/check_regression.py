...
def check_category_regressions(current_results: list, baseline: dict,
...
def display_results(regressions: list, category_regressions: list, threshold: float):
    ...
    console.print()
    table = Table(title="Metric Comparison", box=box.ROUNDED, title_style="bold cyan")
    ...
    for r in regressions:
        ...
        table.add_row(r["metric"], f"{r['baseline']:.1f}%", f"{r['current']:.1f}%", delta_str, status)

    console.print(table)

    if category_regressions:
        console.print()
        cat_table = Table(title="Per-Category Correctness", box=box.SIMPLE, title_style="bold magenta")
        ...
        for r in category_regressions:
            ...
            cat_table.add_row(
                ...
                f"{r['baseline_correctness']:.1f}%", f"{r['current_correctness']:.1f}%",
                delta_str, status,
            )
        ...
        for r in regressions_found:
            if r["key"] == "retrieval_hit_rate":
                console.print(Panel(
                    ...
                    title="[bold red]Regression: Retrieval[/]", border_style="red",
                ))
            elif r["key"] == "avg_faithfulness":
                console.print(Panel(
                    f"[bold]Faithfulness dropped {abs(r['delta']):.1f}%[/]\n\n"
                    "Likely causes:\n"
                    "• System prompt was loosened\n"
                    "• Temperature was raised\n"
                    "• Context is noisier\n\n"
                    "Fix: check SYSTEM_PROMPT and temperature in rag.py",
                    title="[bold red]Regression: Faithfulness[/]", border_style="red",
                ))
...
def main():
    ...
    if not os.path.exists(args.baseline):
        ...
        console.print("[dim]Run eval_harness.py --save-baseline first[/]")
        sys.exit(1)
